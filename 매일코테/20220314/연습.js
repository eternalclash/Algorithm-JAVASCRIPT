function solution(relation) {
    ...
    let combinations = [];
    for (let i = 0; i < idxArr.length; i++){ combinations.push(...getCombination(idxArr, i + 1)); }
...
const checkUniqueness = (arr, comb) =>
{
    ...
    comb.forEach((v1, i1) => {
        let set = new Set();
        arr.forEach((v2, i2) => { set.add(v1.map(e => v2[e]).join(',')); });
 ...
 const getCombination = (arr, num) => {
    const results = [];
     ...
     arr.forEach((fixed, index, origin) => {
         ...
         const attached = combinations.map(v => [fixed, ...v]); results.push(...attached);
     });
    return results;
}
